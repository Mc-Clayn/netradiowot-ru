...
def build_flash():
	...
	for dirPath, _, fileNames in os.walk(AS3_FOLDER):
	...
	for dirPath, fileName, logName in files:
		...
		try:
			subprocess.check_call([CONFIG.software.animate, '-e', jsflFile, '-AlwaysRunJSFL'], stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
...
# region selection (wg or lesta)
BUILD_WG = 'wg' in sys.argv
BUILD_LESTA = 'lesta' in sys.argv

# validate region selection
if BUILD_WG and BUILD_LESTA:
	raise Exception('Cannot build both wg and lesta at the same time')
if not BUILD_WG and not BUILD_LESTA:
	raise Exception('Must specify either wg or lesta parameter')

# set as3 folder based on region
AS3_FOLDER = 'as3_wg' if BUILD_WG else 'as3_lesta'
REGION = 'wg' if BUILD_WG else 'lesta'
BUILD_DIR = 'build_wg' if BUILD_WG else 'build_lesta'
CONFIG_FILE = 'build_{}.json'.format(REGION)

logger.info('Building for region: %s (AS3 folder: %s)', REGION, AS3_FOLDER)

assert os.path.isfile(CONFIG_FILE), 'Config not found: ' + CONFIG_FILE
...
if os.path.isdir('{}/bin'.format(AS3_FOLDER)):
	copytree('{}/bin'.format(AS3_FOLDER), 'temp/res/gui/flash')
copytree('python', 'temp/res/scripts/client', ignore=shutil.ignore_patterns('*.py'))
with open('temp/meta.xml', 'w') as fh:
	fh.write(META)

# create package
zipFolder('temp', '{}/{}'.format(BUILD_DIR, PACKAGE_NAME))
...
if COPY_INTO_GAME:
	...
	shutil.copy2('{}/{}'.format(BUILD_DIR, PACKAGE_NAME), WOT_PACKAGES_DIR)
...
cleanup_list.add('{}/DataStore'.format(AS3_FOLDER))
