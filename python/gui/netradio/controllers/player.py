...
from .._constants import CONSOLE_PLAYER, PLAYER_COMMANDS, PLAYER_STATUS, TAGS_UPDATE_INTERVAL, USER_AGENT, MAX_RESTART_ATTEMPS
...
class PlayerController(object):
	...
	def toggleRadio(self, channelNum=None):
		...
		if not g_controllers.channel.inited:
			BigWorld.callback(0.1, lambda: self.toggleRadio(channelNum))
			return
	...
	def setChannel(self, channelIdx):
		if channelIdx != self.__currentChannel:
			...
			if self.__status == PLAYER_STATUS.PLAYING:
				...
				try:
					channel = g_controllers.channel.channels[channelIdx]
					self.__tag = channel.get('displayName')
					self.__channelName = channel.get('displayName')
					self.__executePlayerCommand(PLAYER_COMMANDS.PLAY, channelIdx)
					
					# Trigger UI updates
					g_eventsManager.onRadioTagChanged()
					g_eventsManager.onRadioStatusUpdated()
					
					# Update tags for the new channel
					self.__updateRadioTags()
				except IndexError:
					pass
			else:
				# Even when stopped, update the channel information for UI consistency
				try:
					channel = g_controllers.channel.channels[channelIdx]
					self.__tag = channel.get('displayName')
					self.__channelName = channel.get('displayName')
					
					# Trigger UI updates to reflect the new channel selection
					g_eventsManager.onRadioTagChanged()
				except IndexError:
					pass
	...
	def __grabChannelTag(self):
		...
		if not tagUrl:
			# If no tags_url, use the channel name as the tag
			channelName = g_controllers.channel.channels[self.channelIdx].get('displayName', '')
			if channelName != self.tag:
				self.__tag = channelName
				g_eventsManager.onRadioTagChanged()
			return
