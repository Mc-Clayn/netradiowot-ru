...
from .._constants import (PLAYER_STATUS, BROADCAST_INTERVAL, BATTLE_INJECTOR_UI, DEFAULT_BATTLE_MESSAGE_COLOR, DEFAULT_BATTLE_MESSAGE_LIFETIME)
...
class BattleController(object):
	...
	def showControlsMessage(self):
		...
		for tip_number in range(1, 4):
			self.__showInfoMessage(l10n('battle.tips.control%s' % str(tip_number)).format(
				toggleRadio=parseKeyValueFull(g_dataHolder.settings['keyBindings']['toggleRadio']),
