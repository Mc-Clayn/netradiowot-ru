...
class DataHolder(object):
	...
	def initConfigOnStart(self, callback):
		...
		if parsedFromNet:
			...
			LOG_DEBUG('Failed to parse config from URL: {}, falling back to default'.format(CONFIG.CONFIG_URL))
			self.__config = DEFAULT_CONFIG
			self.createConfigCache()
		callback(True)
	...
	def __parseConfig(self, callback=None):
		...
		if not CONFIG.CONFIG_URL:
			LOG_DEBUG('DataHolder.__parseConfig', 'No CONFIG_URL provided, using default config')
			callback(False)
			return
		status, data = yield lambda callback: fetchURL(url=CONFIG.CONFIG_URL, callback=callback, timeout=5.0, headers={'User-Agent': USER_AGENT})
		if not status:
			result = False
		...
		try:
			if data:
				...
				LOG_ERROR('DataHolder.__parseConfig', 'Empty data received')
				result = False
		except: #NOSONAR
