...
class NetRadioLobbyView(NetRadioLobbyViewMeta):
	...
	def _dispose(self):
		...
		g_eventsManager.onRadioStatusUpdated -= self.__onRadioStatusUpdated
	...
	def radioPause(self):
		...
		g_controllers.player.toggleRadio()
	...
	def __onRadioStatusUpdated(self):
		"""sync radio status (playing/stopped)"""
		return self.as_setStateS(self.__generateStateCtx())
	...
	def __generateHotkeysCtx(self):
		...
		hotkeys = []
		keySetNames = ['toggleRadio', 'volumeUp', 'volumeDown', 'nextChannel', 'previosChannel']
	...
	def __generateSettingsCtx(self):
		...
		settings = []
		settingsFields = ['saveVolume', 'saveChannel', 'muteOnVoip', 'autoPlay',
